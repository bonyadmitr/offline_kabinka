...
2. **Подсказка установки** (`ui/install-hint.ts`) — закрываемый баннер + блок в настройках. Шаги выбираются по платформе через `detectPlatform()` (`ios`/`android`/`desktop`, эвристика по `userAgent`/`maxTouchPoints`, см. ниже).

---

## Установка PWA — инструкции по платформам

`detectPlatform()` (`src/ui/install-hint.ts`) определяет платформу и подбирает
конкретные шаги (i18n-ключи `install.stepsIos` / `stepsAndroid` / `stepsDesktop`):

| Платформа | Как определяется | Что показывается |
|---|---|---|
| **iOS Safari** | `/iPad\|iPhone\|iPod/` в UA, либо iPadOS 13+ (`Macintosh` + `maxTouchPoints > 1`) | `beforeinstallprompt` на iOS не существует → пошаговая инструкция: «Нажмите „Поделиться“ (квадрат со стрелкой ↑) внизу → „На экран „Домой“"» |
| **Android Chrome** | `/Android/` в UA | Если пойман `beforeinstallprompt` — кнопка «Установить» (`promptInstall()`); как фолбэк в настройках — шаги: «Меню „⋮“ вверху справа → „Установить приложение“» |
| **Desktop** | всё остальное | Аналогично: кнопка «Установить» при наличии `beforeinstallprompt`, иначе шаги: «В адресной строке — значок установки „⊞“, или Меню „⋮“ → „Установить…“» |

Логика: где браузер даёт `beforeinstallprompt` (Android/desktop Chromium), баннер
показывает рабочую кнопку установки, а пошаговую инструкцию оставляет фолбэком в
настройках (`renderInstallHelp`). Где события нет (iOS, прочие движки) — баннер сразу
показывает шаги для текущей платформы. Баннеры идут через общий
`ui/banner-stack.ts`, чтобы не перекрывать оффер офлайн-пакета.
...
## Карта: нет app-управляемого кеша тайлов

Тонкость, важная для UX и для экрана «Размер приложения»: **отдельного,
app-управляемого хранилища тайлов карты, которое можно было бы «почистить», нет.**

- **Онлайн** (карта ещё не скачана пакетом): MapLibre тянет `minsk.pmtiles`
  range-запросами (HTTP `Range`) прямо из сети, и они оседают в **браузерном
  HTTP-кеше** — им управляет браузер, не приложение.
- `minsk.pmtiles` **не в precache** SW: в `vite.config.ts` `globPatterns` ловит
  только `*.{js,css,html,svg,png,woff2,json}` + `fonts/**/*.pbf`, а `globIgnores`
  явно содержит `**/*.pmtiles`.
- `minsk.pmtiles` **не в `runtimeCaching`**: единственное правило там — `CacheFirst`
  для `https://kabinka.by/storage/` (полноразмерные **фото**, кэш `photos`). Тайлов
  карты оно не касается.
- **Офлайн** карта работает не через этот кеш, а через **IDB-блоб** (`minsk`),
  скачанный отдельным пакетом и читаемый `IDBBlobSource`.

Следствия: «временного» кеша карты в смысле приложения не существует, поэтому
`clearTransient()` его не трогает (он чистит только Workbox-кэш `photos`), и онлайн-
тайлы карты **не входят** в подсчёт «Размера приложения» (`estimateUsage()` считает
только blob карты из IDB, blob превью, shell, данные и фото). См. также
[проблему 16](problems-and-solutions.md).
